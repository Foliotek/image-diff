...
function ImageDiff(o) {
  
}
ImageDiff.useGm = function(val) {
  useGraphicsMagick = val;
  if (val) {
    gm = require('gm');
  }
  else {
...
ImageDiff.extractDifference = function (output) {
  ...
  var r = /all: (\d+(?:\.\d+)?(?:[Ee]-?\d+)?) \((\d+(?:\.\d+)?(?:[Ee]-?\d+)?)\)/;
  if (useGraphicsMagick) {
    r = /Total:\s(\d+.?\d+)\s+(\d+.?\d+)/i;
  }
  var resultInfo = output.match(r);
  if (!resultInfo) {
...
ImageDiff.createDiff = function (options, cb) {
  ...
  if (useGraphicsMagick) {
   ...
   diffArgs = [
    ...
    diffArgs.concat(options.shadow ? [] : ['-compose', 'Src'])
  ...
  bufferedSpawn(diffCmd, diffArgs, function processDiffOutput (err, stdout, stderr) {
